				...
				<Form layout="vertical">
					<Form.Item label="班级名：">
						{getFieldDecorator('grade_name', {
							initialValue: (data.grade_name) ? (data.grade_name) : '',

							rules: [{ required: true, message: 'Please input the title of collection!' }],
						})(<Input disabled={!f} />)}
					</Form.Item>
					<Form.Item label="教室号：" placeholder="请选择教室号">
						{getFieldDecorator("room_id", {
							initialValue: (data.room_text) ? (data.room_text) : ""
					...
					<Form.Item label="课程号：" placeholder="课程号">
						{getFieldDecorator('subject_id', {
							initialValue: (data.subject_text) ? (data.subject_text) : '',
							rules: [{ required: true, message: 'Please input the title of collection!' }],
						})(<Select style={{ width: 472, margin: "0px 3px" }}>
